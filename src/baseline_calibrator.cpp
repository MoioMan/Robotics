...
static double gear_ratio = 0.02568;
                             ...
                             const nav_msgs::Odometry::ConstPtr& msg) 
{
    ...
    //if (abs(v_right - v_left) < 10e-6 && abs(v_right - v_left) != 0.0) ROS_INFO("Wheel speed difference: %f",abs(v_right - v_left));

	if(abs(w_z) > 10e-6 && abs(v_right - v_left) > 10e-2)
	{
        double new_app_baseline = (v_right - v_left) / w_z;

        if (new_app_baseline > 1.5) {
            ROS_INFO("v_left: %f, v_right: %f, diff: %f", v_left, v_right, abs(v_right - v_left));
            ROS_INFO("new_app: %f", new_app_baseline);
        }
